...
namespace fx {
  ...
  auto write_bytes_to_file(const std::filesystem::path& out_path, const std::vector<uint8_t>& bytes)
  {
    ...
    -> std::enable_if_t<std::is_same_v<std::ranges::range_value_t<R>, uint8_t>>
  {
    ...
    write_bytes_to_file(out_path, v);
  }
  ...
  inline constexpr to_file_fn write_byte_range;
} // fx